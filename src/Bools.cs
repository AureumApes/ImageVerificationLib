...
namespace ImageVericationLib
{
    public class Bools
    {
        ...
        public static bool CheckType(Image image, String type){
            if(type == "JPEG" && image.RawFormat.Equals(ImageFormat.Jpeg)) 
                return true;
            if(type == "BMP" && image.RawFormat.Equals(ImageFormat.Bmp))
                return true;
            if(type == "PNG" && image.RawFormat.Equals(ImageFormat.Png)) 
                return true;
            return false;
        }
